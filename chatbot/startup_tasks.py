...
def subjectDetails():
    file_path = os.path.join(settings.BASE_DIR, 'chatbot', 'data', 'UniqueFacultySub.json')
...
def scrape_faculty_details(faculty_url,facultyName,facultySubj):
    ...
    try:
        ...
        logger.error("Error in scrape_faculty_details for URL %s: %s", faculty_url, str(e))
...
def setup_vector_database():
    ...
    try:
        ...
        db_store.faculty_db = WeaviateVectorStore(client=client, index_name=faculty_collection_name, text_key='text', embedding=embeddings)
        db_store.eecs_db = WeaviateVectorStore(client=client, index_name=eecs_collection_name, text_key='text', embedding=embeddings)
